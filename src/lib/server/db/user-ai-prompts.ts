...
export async function getUserAiPrompts(
  db: Db,
  userId: string,
): Promise<UserAiPrompts> {
  const [row] = await db
    .select()
    .from(userAiPrompts)
...
export async function upsertUserAiPrompts(
  db: Db,
  userId: string,
  prompts: Partial<UserAiPrompts>,
) {
  ...
  if (existing.length === 0) {
    ...
    await db
      ...
      .set({
        ...
        ...(prompts.reportPrompt !== undefined && {
