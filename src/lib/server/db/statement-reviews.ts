import {
  ...
  StatementSource,
} from "$lib/enums/statement-review";
import { and, eq } from "drizzle-orm";

import type { getDb } from "./index";
import { statementReviews } from "./schema";
...
export async function updateStatementReviewStatus(
  ...
  await db
    ...
    .set({
      status,
      ...(extra?.extractedJson !== undefined && {
        extractedJson: extra.extractedJson,
      }),
      ...(extra?.transactionCount !== undefined && {
        transactionCount: extra.transactionCount,
      }),
      ...(extra?.duplicateCount !== undefined && {
        duplicateCount: extra.duplicateCount,
      }),
      ...(extra?.errorMessage !== undefined && {
        errorMessage: extra.errorMessage,
      }),
      ...(status === StatementReviewStatus.Applied && {
        appliedAt: new Date(),
      }),
    })
...
export async function getStatementReviewById(
  ...
  return db.query.statementReviews.findFirst({
    where: and(
      eq(statementReviews.id, reviewId),
      eq(statementReviews.userId, userId),
    ),
  });
}

export async function getStatementReviewsByUser(
  db: ReturnType<typeof getDb>,
  userId: string,
) {
...
export async function saveApprovedTransactions(
  ...
  await db
    .update(statementReviews)
    .set({ approvedJson, status: StatementReviewStatus.Ready })
    .where(eq(statementReviews.id, reviewId));
}
