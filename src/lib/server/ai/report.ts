...
function formatCurrency(cents: number): string {
  return toReais(cents).toLocaleString("pt-BR", {
    style: "currency",
    currency: "BRL",
  });
}
...
function formatTagTotals(
  tags: Array<{ name: string; expense: number }>,
): string {
  const withSpend = tags
    .filter((t) => t.expense !== 0)
    .sort((a, b) => b.expense - a.expense);
  if (withSpend.length === 0) return "nenhuma tag com gasto";
  return withSpend
    .map((t) => `${t.name}: ${formatCurrency(t.expense)}`)
    .join(", ");
}
