      ...
      transactions: {
        ...
        items: {
          ...
          properties: {
            ...
            description: {
              type: "string",
              description: "Descrição como aparece no documento",
            },
            amount: {
              type: "number",
              description:
...
const EXTRACT_TOOL = {
  ...
  description:
    "Extrai e categoriza as transações financeiras listadas no documento PDF",
};
...
export async function extractTransactionsFromPdf(
  ...
  throw new Error(
    `O provider ${input.provider} não suporta documentos (upload de PDF).`,
  );
}
